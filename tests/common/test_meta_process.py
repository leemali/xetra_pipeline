...
class TestMetaProcessMethods(unittest.TestCase):
    ...
    def test_update_meta_file_meta_file_ok(self):
        ...
        # Expected results
        date_list_old = ['2022-02-18', '2022-02-19']
        date_list_new = ['2022-02-20', '2022-02-21']
        ...
        #print(date_list_result)
        #print(df_meta_result[
          # MetaProcessFormat.META_PROCESS_COL.value])
        #df_meta_result[MetaProcessFormat.META_PROCESS_COL.value] = df_meta_result[MetaProcessFormat.META_PROCESS_COL.value].fillna(method='ffill')
        #df_meta_result[MetaProcessFormat.META_PROCESS_COL.value] = df_meta_result[MetaProcessFormat.META_PROCESS_COL.value].fillna(method='bfill')
        proc_date_list_result = list(pd.to_datetime(df_meta_result[
            MetaProcessFormat.META_PROCESS_COL.value])\
                .dt.date)
        #print(proc_date_list_result)
        # Test after method execution
        self.assertEqual(date_list_exp, date_list_result)
        self.assertEqual(proc_date_list_exp, proc_date_list_result)
        # Cleanup after test
    ...
    def test_return_date_list_meta_file_ok(self):
        ...
        print( f"{MetaProcessFormat.META_SOURCE_DATE_COL.value}")
        print(meta_key)
        print(meta_content)
        self.s3_bucket.put_object(Body=meta_content, Key=meta_key)
        first_date_list = [
